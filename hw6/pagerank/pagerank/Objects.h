...
public:
	...
	void bubbleSort(Node<T> *cur, int ctr, int ordered) {
		if (cur != NULL) {
			if (cur == head) {
				if (cur->data > cur->next->data) {
					...
					head = temp;
					if (ordered < ctr) {
						bubbleSort(cur, ctr, ordered);
					}
				}
				else {
					if (ordered < ctr) {
						bubbleSort(cur->next, ctr, ordered + 1);
					}
				}
			}
			else if (cur == last) {
				if (cur->data > cur->prev->data) {
					bubbleSort(head, ctr - 1, 0);
				}
			}
			else {
				if (cur->data > cur->next->data) {
